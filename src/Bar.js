...
const Bar = () => {
  ...
  const menus = [
    ...
    { name: "Employee List", path: "/employee", icon: PiUserListBold },
    { name: "Attendance ", path: "/attendance", icon: MdFormatListBulleted },
    { name: "Leave List", path: "/leave", icon: LuListX },
    { name: "Payroll", path: "/payroll", icon: FaMoneyCheckDollar },
          ...
          {menus.map((menu, i) => (
            <NavLink
              ...
              className={`group flex items-center text-sm gap-3.5 font-medium p-2 hover:bg-gray-800 rounded-md mt-5 ${
                location.pathname === menu.path ? "bg-[#E1FF3C] text-black" : ""
              }`}
              key={i}
            >
